...
interface NoticeRecipientPayload {
  id?: number;
  roleId: number;
...
const getNoticeById = async (id: number): Promise<any> => {
  const query = `
    SELECT
      t1.id,
      t1.title,
...
const addNewNotice = async (payload: NoticePayload): Promise<number> => {
  const now = new Date();
  const {
    title,
    status,
...
const updateNoticeById = async (payload: NoticePayload): Promise<number> => {
  const now = new Date();
  const {
    id,
    title,
...
const addNoticeRecipient = async (payload: NoticeRecipientPayload): Promise<number> => {
...
const updateNoticeRecipient = async (payload: NoticeRecipientPayload): Promise<number> => {
...
const deleteNoticeRecipient = async (id: number): Promise<number> => {
...
const getNoticeRecipientById = async (id: number): Promise<any> => {
  const query = `
    SELECT
      id,
      role_id AS "roleId",
...
const manageNoticeStatus = async (payload: NoticeStatusPayload): Promise<number> => {
