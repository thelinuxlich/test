...
const getClassTeachers = async (): Promise<any[]> => {
  const query = `
    SELECT
      t1.id,
...
const addClassTeacher = async (payload: ClassTeacherPayload): Promise<number> => {
...
const getClassTeacherById = async (id: number): Promise<any> => {
  const query = `
    SELECT
      id,
      class_name AS class,
...
const updateClassTeacherById = async (payload: ClassTeacherPayload): Promise<number> => {
...
const findAllTeachers = async (): Promise<any[]> => {
  const teacherRole = 2;
  const query = `
    SELECT id, name
    FROM users
