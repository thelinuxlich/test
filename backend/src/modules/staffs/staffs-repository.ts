...
const getAllStaffs = async (payload: StaffPayload): Promise<any[]> => {
...
const getStaffDetailById = async (id: number): Promise<any> => {
  const query = `
    SELECT
      t1.id,
      t1.name,
...
const reviewStaffStatus = async (payload: StaffStatusPayload): Promise<number> => {
  const now = new Date();
