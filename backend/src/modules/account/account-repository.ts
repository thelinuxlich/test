...
const changePassword = async (payload: ChangePasswordPayload, client: any): Promise<void> => {
...
const getUserRoleNameByUserId = async (id: number, client: any): Promise<string> => {
  const query = `
...
const getStudentAccountDetail = async (userId: number): Promise<any> => {
...
const getStaffAccountDetail = async (userId: number, userRoleId: number): Promise<any> => {
  const query = `
    SELECT
      t1.id,
      t1.name,
