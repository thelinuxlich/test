...
> ⚠️ **Note:** All problems must be implemented within the current project structure — do **not** build standalone apps. Each task should enhance or fix the existing system.
...
- **Objective**: Implement and integrate a Go microservice within the existing project structure to generate PDF reports for students by consuming the existing Node.js backend API.
...
- **Objective**: Containerize the existing frontend, backend, and database using Docker and Docker Compose — do not create a new setup from scratch.
